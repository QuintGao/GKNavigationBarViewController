...
- (void)panGestureAction:(UIPanGestureRecognizer *)gesture {
    ...
    if (gesture.state == UIGestureRecognizerStateBegan) {
        if (self.isGesturePush) { // push操作
            ...
            if (self.navigationController.gk_translationScale) {
...
- (void)popScrollEnded:(BOOL)finished {
...
- (id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC {
    ...
    CGFloat isScale = self.navigationController.gk_translationScale;
    if ((isScale || self.pushTransition) && operation == UINavigationControllerOperationPush) {
        return [[GKPushTransitionAnimation alloc] initWithScale:isScale];
    }
        
    if ((isScale || self.popTransition) && operation == UINavigationControllerOperationPop) {
        return [[GKPopTransitionAnimation alloc] initWithScale:isScale];
    }
...
- (id<UIViewControllerInteractiveTransitioning>)navigationController:(UINavigationController *)navigationController interactionControllerForAnimationController:(id<UIViewControllerAnimatedTransitioning>)animationController {
    ...
    if (self.pushTransition && [animationController isKindOfClass:[GKPushTransitionAnimation class]]) {
        return self.pushTransition;
    }
    
    if (self.popTransition && [animationController isKindOfClass:[GKPopTransitionAnimation class]]) {
